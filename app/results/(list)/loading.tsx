import { SkeletonScreen } from "@/components/skeletons/PageSkeletons";
import { HatchOverlay } from "@/components/paddock/HatchOverlay";
import { Skeleton } from "@/components/ui/skeleton";

/**
 * Groupe `(list)` — sans effet sur l'URL — pour que cette frontière de
 * chargement ne couvre pas `/results/[season]` ni `/results/[season]/[round]`,
 * qui appellent `notFound()`. Voir components/skeletons/README.md.
 */
export default function Loading() {
  return (
    <SkeletonScreen label="Chargement des saisons">
      <section className="relative overflow-hidden border-b border-border px-6 py-14 md:px-12">
        <HatchOverlay />
        <div className="relative flex max-w-2xl flex-col gap-5">
          <div className="flex items-center gap-3">
            <span className="h-[3px] w-8 bg-primary/40" />
            <Skeleton className="h-3 w-56" />
          </div>
          <Skeleton className="h-10 w-52 sm:h-14" />
          <Skeleton className="h-3 w-full max-w-xl" />
        </div>
      </section>
      <section className="px-6 py-10 md:px-12">
        <div className="grid grid-cols-1 gap-px border border-white/8 bg-white/8 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="flex flex-col gap-4 bg-background p-7">
              <Skeleton className="h-8 w-20" />
              <Skeleton className="h-4 w-32" />
              <Skeleton className="h-4 w-40" />
              <Skeleton className="mt-auto h-4 w-24" />
            </div>
          ))}
        </div>
      </section>
    </SkeletonScreen>
  );
}
